...
public void save(Book book) {
  this.jdbcTemplate.update("INSERT INTO book(title, author, year, person_id) VALUES(?, ?, ?,?)", 
...
{
	return this.jdbcTemplate.query("SELECT person.* FROM book JOIN person ON book.person_id = person.id WHERE book.id=?", 
			new Object[]{id}, new BeanPropertyRowMapper<>(Person.class)).stream().findAny();
}

public void release(int id)
{
	this.jdbcTemplate.update("UPDATE book SET person_id=1 WHERE id=?", id);
}

public void assign(int id, Person selectedPerson)
{
	this.jdbcTemplate.update("UPDATE book SET person_id=? WHERE id=?",  selectedPerson.getId(), id);
}
